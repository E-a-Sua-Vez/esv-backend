...
export class PlanActivationService {
  ...
  public async getValidatedPlanActivation(validated: string): Promise<PlanActivation[]> {
    ...
    if (planActivations.length > 0) {
      for (let i = 0; i < planActivations.length; i++) {
        planActivations[i].business = await this.businessService.getBusinessById(
          planActivations[i].businessId
        );
        if (planActivations[i].paymentId) {
          planActivations[i].payment = await this.paymentService.getPaymentById(
            planActivations[i].paymentId
          );
        }
      }
    }
  ...
  public async createPlanActivation(
    ...
    if (!businessId || !planId || !planPayedCopy || !paymentMethod || termsAccepted === undefined) {
      throw new HttpException(
        `No hay suficientes datos para activar el plan`,
    ...
    const validatedPlanActivation = await this.getValidatedPlanActivationByBusinessId(
      businessId,
      'false'
    );
    if (validatedPlanActivation) {
      throw new HttpException(
        `Ya tienes una solicitud de activacion de plan pendiente por validar`,
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    const planActivationCreatedEvent = new PlanActivationCreated(
      new Date(),
      planActivationCreated,
      { user }
    );
  ...
  public async update(user: string, planActivation: PlanActivation): Promise<PlanActivation> {
    ...
    const planActivationUpdatedEvent = new PlanActivationUpdated(
      new Date(),
      planActivationUpdated,
      { user }
    );
  ...
  public async validate(
    ...
    try {
      if (!id || !businessId || !planId || amount === undefined || !paymentNumber) {
        throw new HttpException(
          `No hay suficientes datos para validar el plan`,
          HttpStatus.BAD_REQUEST
        );
      }
      if (amount < 0) {
      ...
      if (!payment || !payment.id) {
        throw new HttpException(
          `No existe pago valido para activar el plan`,
          HttpStatus.BAD_REQUEST
        );
      }
      const plan = await this.planService.getPlanById(planId);
  ...
  public async desactivate(user: string, planId: string): Promise<PlanActivation> {
    try {
      if (!planId) {
        throw new HttpException(
          `No hay suficientes datos para validar la activacion del plan`,
          HttpStatus.BAD_REQUEST
        );
      }
      ...
      planActivation.permissions = await this.planService.desactivatedPermissionsForPlan(
        planActivation.planId
      );
